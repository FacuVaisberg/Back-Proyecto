...
export default class RecetaService
    ...
    getAllDisponibles = async (id)=> {
            ...
            try {
                ...
                .query('SELECT M.NombreMedicamento, UM.Nombre as NombreMedico, UP.Nombre as NombrePaciente, FechaCreacion, P.Direccion, P.Dni, U.Telefono FROM Receta R INNER JOIN Medicamentos M ON R.IdMedicamento = M.IdMedicamentos INNER JOIN Usuario UM ON R.IdMedico = UM.IdUsuario INNER JOIN Usuario UP ON R.IdPaciente = UP.IdUsuario INNER JOIN Paciente P ON P.IdUsuario = UP.IdUsuario INNER JOIN Usuario U  ON U.IdUsuario = UP.IdUsuario')
